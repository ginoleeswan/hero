...
.foot .row{display:flex;align-items:baseline;gap:14px;}
...
${extra}`;
  return `<!doctype html><html><head><meta charset="utf-8"><style>${css}</style></head><body>${svgFilterDefs()}<div class="page">${plate ? `<img class="bgplate" src="${plate}"><div class="veil"></div>` : ''}<div class="dots"></div><div class="grain"></div>${inner}<div class="foot"><div class="row"><span class="wm">mythique</span><span class="at">@mythiqueapp</span></div><div class="disc">${DISCLAIMER}</div></div></div></body></html>`;
}
