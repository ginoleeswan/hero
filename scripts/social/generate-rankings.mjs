  ...
  const scope =
    opts.alignment === 'bad'
      ...
      : opts.alignment === 'good'
        ? 'HEROES'
        : opts.publisher
          ? `${opts.publisher.replace(/\s+comics?$/i, '').toUpperCase()} CHARACTERS`
          : 'CHARACTERS';
  const title = opts.title || `TOP ${opts.count} ${mw} ${scope}`;
  const sub =
    opts.by === 'fame' ? 'RANKED BY MYTHIQUE FAME SCORE' : `RANKED BY ${opts.by.toUpperCase()}`;
  return { title, sub };
}
...
async function fetchRanking(sb, opts) {
  ...
  if (opts.by === 'fame') order = 'fame_score.desc,issue_count.desc.nullslast';
    ...
    filter += `&${metric}=not.is.null`;
    order = `${metric}.desc.nullslast,fame_score.desc,issue_count.desc.nullslast`;
  }
  const rows = await sb.rest(
    `heroes?select=${cols.join(',')}${filter}${REAL_PERSON_FILTER}&order=${order}&limit=${opts.count}`,
  );
  return rows.map((r, i) => ({
    ...r,
    rank: i + 1,
    value: composite ? Number(r[metric] ?? 0).toFixed(1) : (r[metric] ?? 0),
  }));
}
...
function rankRow(e) {
  ...
  const bg = top
    ? 'background:rgba(224,168,62,.10);border:2px solid rgba(224,168,62,.45);'
...
function slideRanks(t, rows, F, showTitle) {
  ...
  const head = showTitle
    ? `<div class="h1" style="font-size:52px;margin-bottom:30px">${esc(t.title)}</div>`
    : '';
  return slide(
    F,
    `<div class="body">${head}<div class="full" style="display:flex;flex-direction:column;justify-content:space-evenly;flex:1;min-height:0">${list}</div></div>`,
  );
}
...
function caption(t, rows) {
  const top3 = rows
    ...
    .map((e, i) => `${i + 1}. ${e.name}`)
    .join('   ');
  // A ranking has no single page of its own — it is a filter, not an entity —
  // so the link goes to the character at number one, who is the person the post
  // is most about and whose page certainly exists. Better a real subject than
  // the homepage, which is what this said before and where 89% of arrivals stop.
  const lead = rows[0];
  return [
    ...
    lead?.id
      ? `Start with ${lead.name}: ${postUrl(`character/${encodeURIComponent(lead.id)}`, { campaign: 'rankings', content: lead.id })}`
      : `Full leaderboard on mythique.app`,
    ``,
...
async function main() {
  ...
  if (!METRIC_WORD[opts.by]) {
    console.error('--by must be one of:', Object.keys(METRIC_WORD).join(', '));
    process.exit(1);
  }
  ...
  if (!rows.length) {
    console.error('No characters matched that filter.');
    process.exit(1);
  }
