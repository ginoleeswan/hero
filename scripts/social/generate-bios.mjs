...
function tagline(h) {
  ...
  const aliases = (Array.isArray(h.aliases) ? h.aliases : []).filter(
    (a) =>
      a &&
      a.trim().length > 3 &&
      a.length <= 24 &&
      a.toLowerCase() !== name &&
      !a.toLowerCase().startsWith(name) &&
      !/[0-9]/.test(a),
  );
  return (
    aliases.find((a) => /^the\s/i.test(a)) ||
    aliases[0] ||
    h.publisher ||
    'Mythique'
  ).toUpperCase();
...
function slideDossier(h, F, year) {
  const fields = [
    ...
    [
      'FIRST APPEARANCE',
      year ? `${h.first_appearance || ''} (${year})`.trim() : h.first_appearance,
    ],
    ['RACE', h.race],
  ...
  const rows = fields
    .map(
      ([l, v]) => `<div style="margin-bottom:40px">
      <div style="font-size:30px;letter-spacing:4px;color:${GOLD};margin-bottom:8px">${l}</div>
      <div style="font-family:'S';font-size:52px;color:${CREAM};line-height:1.12">${esc(v)}</div></div>`,
    )
    .join('');
  return slide(
    F,
    `<div class="body"><div class="h1">DOSSIER</div>
...
function slideNumbers(h, F, year) {
  ...
  const cells = tiles
    .map(
      (
        t,
      ) => `<div style="width:420px;height:320px;margin:18px;border-radius:40px;border:2px solid rgba(224,168,62,.32);background:rgba(245,235,220,.04);display:flex;flex-direction:column;align-items:center;justify-content:center">
      <div class="pop" style="font-size:120px;color:${CREAM}">${t[0]}</div>
      <div style="font-size:32px;letter-spacing:3px;color:${GOLD};margin-top:10px">${t[1]}</div></div>`,
    )
    .join('');
  return slide(
    F,
    `<div class="body"><div class="h1">BY THE NUMBERS</div>
...
function slideStats(h, F) {
  ...
  return slide(
    F,
    `<div class="body"><div class="h1">POWER STATS</div><div class="full">${rows}</div></div>`,
  );
}
...
function slideAbilities(h, F) {
  ...
  const chips = powers
    .map(
      (p) =>
        `<div style="display:inline-block;margin:14px;padding:20px 40px;border-radius:100px;border:2px solid rgba(224,168,62,.42);background:rgba(245,235,220,.05);font-size:48px;color:${CREAM}">${esc(p)}</div>`,
    )
    .join('');
  return slide(
    F,
    `<div class="body"><div class="h1">ABILITIES</div><div class="full">${chips}</div></div>`,
  );
}
...
function thumbRow(label, tone, people) {
  const cells = people
    .map(
      (
        p,
      ) => `<div style="display:inline-block;width:210px;margin:0 8px;text-align:center;vertical-align:top">
      ...
      <div style="font-family:'S';font-size:29px;color:${CREAM};margin-top:14px;line-height:1.2">${esc(clip(p.name, 26))}</div></div>`,
    )
    .join('');
...
function slideAssociates(F, groups) {
  ...
  return slide(
    F,
    `<div class="body"><div class="h1">KNOWN ASSOCIATES</div><div class="full">${rows}</div></div>`,
  );
}
...
function slideFacts(h, F, facts) {
  const items = facts
    .map(
      (
        f,
      ) => `<div style="display:flex;align-items:flex-start;gap:28px;margin-bottom:52px;text-align:left">
      <div style="color:${GOLD};font-size:56px;line-height:1.05">◆</div>
      <div style="font-family:'S';font-size:54px;color:${CREAM};line-height:1.26">${f}</div></div>`,
    )
    .join('');
  return slide(
    F,
    `<div class="body"><div class="h1">DID YOU KNOW</div>
...
function buildFacts(h, year, enemies) {
  ...
  if (year)
    out.push(
      `${esc(h.name)} first appeared in <b style="color:${GOLD}">${year}</b>, over <b style="color:${GOLD}">${YEAR - year}</b> years of stories.`,
    );
  const powers = Array.isArray(h.powers) ? h.powers : [];
  if (powers.length)
    out.push(
      `Wields <b style="color:${GOLD}">${powers.length}</b> distinct abilities in the catalogue.`,
    );
  if (enemies.length >= 2)
    out.push(
      `Has clashed with icons like <b style="color:${GOLD}">${esc(enemies[0].name)}</b> &amp; <b style="color:${GOLD}">${esc(enemies[1].name)}</b>.`,
    );
  if (out.length < 3 && h.fame_score != null)
    out.push(`Mythique fame score: <b style="color:${GOLD}">${h.fame_score}/100</b>.`);
...
function caption(h) {
  ...
  const bits = [
    h.movie_count && `${h.movie_count} films`,
    h.issue_count && `${fmt(h.issue_count)} comics`,
  ]
    .filter(Boolean)
    .join('  ·  ');
  // The character's OWN page. A viewer who just watched thirty seconds about
  // this one character was being handed a general feed.
  const url = postUrl(`character/${encodeURIComponent(h.id)}`, {
    campaign: 'bios',
    content: h.id,
  });
  return [
    ...
    ``,
    bits ? `${bits}. Powers, rivalries and every matchup:` : `Powers, rivalries and every matchup:`,
    url,
...
async function withThumbs(rows, n) {
  const picked = (rows || [])
    .filter((r) => r.portrait_url || r.image_md_url || r.image_url)
    .slice(0, n);
  const out = [];
...
async function build(sb, h, F) {
  ...
  const [enemies, allies, family] = await Promise.all([
    withThumbs(enemiesRaw, 4),
    withThumbs(alliesRaw, 4),
    withThumbs(familyRaw, 4),
  ]);
  const groups = [
  ...
  if (year || h.movie_count || h.issue_count || h.wikidata_sitelinks)
    slides.push(slideNumbers(h, F, yr));
