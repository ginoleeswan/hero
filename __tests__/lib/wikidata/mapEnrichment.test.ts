import { mapWorkRow, mapPersonRow, type WorkRow, type MappedTitle } from '../../../src/lib/wikidata/mapEnrichment';
...
describe('mapWorkRow', () => {
  ...
  it('parses a full publication date down to the year', () => {
    const r: WorkRow = { workLabel: 'X', year: '2008-07-18T00:00:00Z', tmdbMovie: '1', tmdbTv: null, igdb: null };
...
describe('mapPersonRow', () => {
  it('flags voice actors vs live performers', () => {
    expect(mapPersonRow('Kevin Conroy', true)).toEqual({ personName: 'Kevin Conroy', role: 'voice_actor' });
    expect(mapPersonRow('Michael Keaton', false)).toEqual({ personName: 'Michael Keaton', role: 'performer' });
  });
});
