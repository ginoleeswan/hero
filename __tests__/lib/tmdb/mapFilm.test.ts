...
describe('mapTmdbDetailsToFilm', () => {
  it('maps core fields and builds image URLs', () => {
    ...
    expect(f.popularity).toBe(88.5);
  });

  it('maps missing popularity to null', () => {
    expect(mapTmdbDetailsToFilm({ id: 1, title: 'X', release_date: null }).popularity).toBeNull();
  });
  ...
  it('keeps providers, top cast, and still URLs', () => {
    ...
    expect(f.cast_members?.[0]).toEqual({
      name: 'Michael Keaton',
      character: 'Batman',
      profile_url: 'https://image.tmdb.org/t/p/w185/mk.jpg',
    });
