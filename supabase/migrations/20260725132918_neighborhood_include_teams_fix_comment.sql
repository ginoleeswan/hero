-- Add teams to the social-web nodes so the focus card can explain WHY two
-- characters are teammates (the rosters they share), rather than only asserting
-- the relationship. ComicVine gives no reason text for enemy/ally ties, but
-- shared team membership is real, specific and already stored.
