-- get_trending_titles_multi: one round-trip for ALL trending-title buckets.
--
-- The Explore home load called get_trending_titles three times — once per bucket
-- (on_screen / coming_soon / streaming) — differing only by p_bucket. This folds
-- them into a single call that tags each row with its bucket, so the client splits
-- the flat result once instead of paying three separate round-trips.
--
-- The per-bucket logic MIRRORS get_trending_titles(p_bucket) exactly (title window
-- ranking, character window ranking, the date/provider predicate). Keep the two in
-- sync. A title that qualifies for two buckets (e.g. on_screen + streaming) appears
-- under each, ranked independently — identical to issuing the two calls separately.
...
grant execute on function public.get_trending_titles_multi(text[], int, int) to anon, authenticated, service_role;
