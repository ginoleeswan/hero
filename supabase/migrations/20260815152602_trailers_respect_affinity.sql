-- "What dropped" must belong to the event, not merely to its week.
--
-- The Nintendo Direct page listed Masters of the Universe, Spider-Man: Brand New
-- Day, Toy Story 5 and Thunderbolts. Not one is Nintendo. Announcements and
-- movers were given an affinity gate; trailers were deliberately exempted on the
-- reasoning that "Trailers published inside the window" is a temporal claim and
-- therefore honest. That reasoning does not survive contact with the page:
-- directly beneath the event's own mark, under a heading that says WHAT DROPPED,
-- a grid of unrelated film trailers reads as the event's slate. Third time this
-- same mistake has been made — the window is not the event.
--
-- Two ways a trailer can belong, and either is enough:
--
--   BY CHANNEL — the video came from one of the event's own channels. Exact,
--     and the only signal that catches something like Percy Jackson, a Disney+
--     show whose characters are not in a Disney-owned publisher.
--   BY CAST — the catalogue links the title to characters from the event's
--     publishers. The only signal available for historical editions, which
--     predate channel_videos entirely.
--
-- NULL affinity still means no filter, which remains right for a general
-- convention: San Diego Comic-Con genuinely is where everyone's trailer drops.
