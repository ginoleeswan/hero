-- admin_audience_breakdown: unnest the visit-splitting window functions.
--
-- 20260915144738 put lag() inside sum() over (...), which Postgres rejects
-- ("window function calls cannot be nested"), so the function errored on
-- every call. The gap flag is now computed in an inner query and summed in
-- the outer one. Everything else is unchanged.
      ...
      'totals', (select json_build_object(
        ...
        'avgMinutes', (select round(coalesce(avg(minutes) filter (where views > 1), 0)::numeric, 1)
          from (
            ...
            from (
              -- Two window passes: the gap flag first, then its running sum
              -- (Postgres does not allow a window call inside another).
              ...
              from (
                select session_id, created_at,
                       case when created_at - lag(created_at) over (partition by session_id order by created_at)
                                 > interval '30 minutes' then 1 else 0 end as starts_visit
                from pv
              ) gaps
            ) g
