-- Add the title synopsis to get_trending_titles so the auto-generated "Right Now"
-- hero (synthesized from the trending slate when no editorial campaign is live)
-- can use a real blurb. Return shape changes → drop first. Body is otherwise the
-- 20260615180000 definition with t.overview threaded through.
...
returns table (
  ...
  backdrop_url text, poster_url text, provider text, overview text,
