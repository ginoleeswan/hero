-- Refine get_trending_heroes: keep recency as the primary order (so the feed
-- leads with the most recent release and feels current), but break ties by the
-- character's popularity (issue_count) so marquee names lead within each title
-- rather than bit players.
