-- Daily "Guess the Hero" puzzle. One recognisable hero per day, deterministic
-- for everyone (Wordle-style). The answer is returned to the client so the
-- comparison can run instantly client-side — like Wordle, the puzzle is
-- honour-system, not a server secret. Anon + authenticated may read it.
--
-- The pool is the 500 most-published enriched heroes (real art, a summary, a
-- known alignment, actual powerstats), then shuffled by a stable hash and
-- indexed by day — so the daily answer is always a recognisable, well-known
-- character, never an obscure stub, and the sequence doesn't repeat for ~500 days.
...
as $$
  ...
  select json_build_object(
    ...
    'hero', json_build_object(
      ...
      'powerstats_total', powerstats_total,
      'powers', powers
    )
  )
  from pick;
