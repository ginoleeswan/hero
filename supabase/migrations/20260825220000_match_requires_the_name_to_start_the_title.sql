...
--
-- ⚠ NOT YET APPLIED. Every other migration here was applied through the Supabase
-- MCP tool, which assigns its own version. This one was written in a session
-- without write access to the project, so the filename's timestamp was chosen by
-- hand. Apply it with `apply_migration`, then rename this file to the version the
-- database records (see CLAUDE.md — the repo has already been through four
-- months of two numbering schemes and does not need a fifth).
--
-- ── what it fixes ───────────────────────────────────────────────────────────
--
