-- Where this event USUALLY runs, so an edition can tell whether its own venue is
-- worth remarking on.
--
-- A venue is only news when it moved. Eighteen of the twenty-one watched events
-- have sat in the same hall for a decade, and a page that draws a map to say
-- "San Diego" about San Diego Comic-Con is furniture — the event's own name
-- already said it. The three that move are the whole point, and they cannot be
-- spotted from a single edition row, only from its siblings. So the reader
-- returns the modal venue alongside this edition's, and the page shows a map
-- exactly when the two differ.
--
-- Modal, not most recent: D23 is Anaheim four times out of eight, which is what
-- "usually" means to a reader. "Last time" would have said Walt Disney World and
-- made 2024's Anaheim look like the anomaly.
--
-- Suppressed rows are excluded — a cancelled show is not evidence about where an
-- event is normally held.
--
-- Everything below the `usual` CTE is unchanged from 20260816091500; it is
-- restated only because CREATE OR REPLACE has no other mode.
