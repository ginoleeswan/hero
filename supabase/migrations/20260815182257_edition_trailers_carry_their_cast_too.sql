-- Same change as trailers_carry_their_cast, on the edition read path.
--
-- The two functions render the same component, so a field added to one and not
-- the other means the archive silently keeps the poster wall while the live page
-- gets the catalogue index. These two have drifted before — the affinity gate
-- had to be applied to each separately — and the only reason they are separate
-- at all is that one reads watched_events and the other reads a frozen row.
