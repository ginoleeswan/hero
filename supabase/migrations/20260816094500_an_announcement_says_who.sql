-- An announcement says WHO, and the page was not showing it.
--
-- The announcements grid was thirteen white cards, each a still, a name, a line
-- of studio copy and a channel — which is the shape of every film grid on the
-- internet, because it is built from what every one of them has. The one asset
-- this app holds that they do not is the catalogue: Ahsoka Tano, Anakin,
-- Grand Admiral Thrawn, matched to the title the studio just announced.
--
-- The trailers block in this same function has returned that cast since it was
-- written, and its component's own comment calls the faces "the reason a reader
-- who came for a trailer stays for a character". The announcements block — which
-- sits ABOVE it and is read first — returned none. This is that same subquery,
-- keyed on the announcement's title instead of the trailer's.
--
-- Five rather than the trailers' six: an announcement card is 264pt wide against
-- the trailer lead's full measure, and a sixth face pushes the strip into a
-- second row.
--
-- Per-clip, and therefore repeated across the clips that share a title, which
-- `groupAnnouncements` folds back down on the client. The alternative is a
-- lateral join to de-duplicate before the aggregate, which costs more to read
-- than the forty rows it would save.
--
-- Everything else is unchanged from 20260816092500; restated because
-- CREATE OR REPLACE has no other mode.
...
as $function$
  ...
  usual as (
    select x.venue_city, x.venue_lat, x.venue_lon
    from public.event_editions x, e
    where x.slug = e.slug
      and x.venue_city is not null
      and x.suppressed_reason is null
    group by x.venue_city, x.venue_lat, x.venue_lon
    order by count(*) desc, x.venue_city
    limit 1
  ),
  ...
  announcements as (
    ...
    from (
      select jsonb_build_object(
        ...
        -- Who the announcement is ABOUT, from our own catalogue. Identical shape
        -- and ordering to the trailers block below, deliberately: the two
        -- sections render the same face strip, so they must agree on who is in
        -- it and in what order.
        'cast_count', (
          select count(*) from public.hero_media_appearances a
          join public.heroes h on h.id = a.hero_id
          where a.title_id = cv.title_id
  ...
  select case when (select count(*) from e) = 0 then null else
    jsonb_build_object(
      'event', jsonb_build_object(
        ...
        'venue_usual_city', (select venue_city from usual),
        'venue_usual_lat', (select venue_lat from usual),
        'venue_usual_lon', (select venue_lon from usual)
      ),
