-- Admin-triggered Wikidata resolve drain, mirroring admin_run_drain. Lets an
-- admin continue the hero->QID resolution from the command center on demand.
create or replace function public.admin_run_wikidata_resolve(p_limit integer default 10)
returns text
language plpgsql
security definer
as $function$
...
begin
  ...
  perform net.http_post(
    url := v_url || '/functions/v1/resolve-wikidata-batch',
