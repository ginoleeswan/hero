-- Fix: count distinct heroes per team in shared-affiliation block.
-- A hero listing the same team twice in heroes.teams[] previously inflated
-- affil_coverage. Using count(distinct h.id) ensures each hero is counted once
-- per team, regardless of duplicate entries in their teams array.
...
begin
  ...
  -- count(distinct h.id) prevents a hero with duplicate team entries from
  -- inflating the coverage count.
  select trim(t), count(distinct h.id) into affil_team, affil_coverage
