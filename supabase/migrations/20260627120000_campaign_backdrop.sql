-- Campaign hero art: expose the linked title's TMDB backdrop (and poster) so the
-- "Right Now" editorial hero can use a purpose-composed wide still instead of
-- cropping a single character's portrait art. Title-linked campaigns get the
-- backdrop; franchise-/hero-only campaigns return null and the UI falls back to
-- character art. Recreated function — only the title columns are added.
-- Drop first: the return shape changes (Postgres can't alter OUT params in place).
...
returns table (
  ...
  backdrop_url text, poster_url text,
