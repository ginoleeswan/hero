...
| Tab bar selected tint | One `systemChromeMaterialDark` blur serves four screens, and the darkest (the Arena's near-black stage) sets the contrast floor. `#e8621a` measured 4.08–4.64:1 against that material — at or below the 4.5 floor, on the one item meant to be the MOST legible in the bar. It read fine on Explore only because the beige page behind the blur lifts the material. `TAB_ACTIVE` (`src/constants/colors.ts`) is the brand orange lifted for small marks: 5.49–7.15:1 across the same range. A tint is not a fill — a 24pt glyph and a 10pt label need more separation than a card does. |
