...
| Title page seam + voice | Same pass as the issue page: `FilmBackdropHeader` takes `bottomClearance` so native can hand it the room the rounded cap consumes, the title/back chevron move from `#fff` to beige, and the native title page + both skeleton states seam with the `SEAM` cap (the header's hairline sits beneath it on native; web keeps the hairline because its wide layout deliberately straddles cards across the seam). Geometry shared via `src/constants/titleGeometry.ts`. |
| Issue page seam + voice | `app/issue/[id].tsx` was the only detail page whose ink→paper edge was a flat 1px hairline instead of the rounded beige `SEAM` cap, and its masthead was `#fff` on ink where the app's voice is beige. Both fixed; the header/skeleton numbers live in `src/constants/issueGeometry.ts`, read by the page and `IssueSkeleton` both (the categoryGeometry rule, applied before this pair could drift). |
...
| Slug-page skeleton alignment | The sharing rule got applied to the three remaining slug pages, and each was wrong in a different way. **Category:** the sheet's own box is pulled up by `SEAM.overlap`, so its `paddingTop` must be the FULL `capHeight` for the first grid row to land `capHeight - overlap` below the stage. It was padded by the difference — subtracting the overlap twice, starting the grid 24pt high everywhere. It also drew one generic masthead where the page has two (a registered universe's 56pt brand logo vs a 40pt title line) and one tagline line where a category wraps to two; all of that is derivable from the slug synchronously, so the screen passes it in. **Event:** the dossier placeholder was ~180pt short — nothing at all for the three-line method paragraph, 40pt for a stat rail that occupies 65, and a 28pt bottom pad where the real stage reserves 60% of the readership curve's height. **House:** the placeholder was drawing the *web* body on native (20pt gutter vs 16, 24pt top pad vs 16, 22pt stack gap vs 18), omitted the `StageSwitch` on both platforms, and drew a full roster rail that only exists above the two-column breakpoint — ~350pt of list that never arrives. |
| Line boxes must be stated, not inherited | The root cause under the event and house misalignments: a `Text` with no `lineHeight` is as tall as the font's own metrics decide, which a placeholder can only approximate. Any text a skeleton mirrors needs an explicit `lineHeight`, and a constant paragraph the skeleton has to match needs a fixed box + `numberOfLines` so its height is a fact. `EventDossier`, `EventIndexList` and `StageSwitch` were converted; their numbers live in `eventGeometry.ts` / `houseGeometry.ts`. |
| Placeholder bars are ink, not line boxes | A bar filling the whole line box reads as a slab; the same bar at glyph height, centred in that box, reads as a line of type. The BOX is what has to match the real layout — that is what sets where everything below starts — so the pattern is a fixed-height `View` with `justifyContent: 'center'` holding a shorter `Skeleton`. See `CATEGORY_INK` / `EVENT_INK`. |
