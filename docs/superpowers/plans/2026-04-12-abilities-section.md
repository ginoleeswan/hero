...
export async function fetchHeroDetails(heroName: string): Promise<HeroDetails> {
  ...
  const powers: string[] | null =
    Array.isArray(result.powers) && result.powers.length > 0
...
function PowerOrb({ name }: { name: string }) {
  ...
  return (
    <View style={styles.orbItem}>
      ...
      <Text style={styles.orbName} numberOfLines={2}>
        {name}
      </Text>
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        >
          ...
          {overflow > 0 && <MoreOrb count={overflow} onPress={() => setExpanded(true)} />}
        </ScrollView>
      )}
    </View>
  );
