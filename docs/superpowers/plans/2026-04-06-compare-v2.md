...
|------|--------|----------------|
| `src/constants/rivals.ts` | Create | Hardcoded heroId → rivalIds[] map |
...
export function getRivals(heroId: string): string[] {
  return RIVALS_MAP[heroId] ?? [];
}
```
...
export default function PickOpponentScreen() {
  ...
  useEffect(() => {
    const rivalIds = getRivals(heroId ?? '');
    ...
    Promise.all([
      ...
      .then(([allHeroes]) => {
        ...
        const filtered = allHeroes.filter((h) => h.id !== heroId);
        setAll(filtered);
    ...
    getHeroById(heroId ?? '').then((row) => {
      ...
      getHeroesByPowerRange(total - margin, total + margin, heroId ?? '').then(setSimilar);
    }).catch(() => {});
  }, [heroId]);
  ...
  const handlePick = (id: string) => {
    router.replace(`/compare/${heroId}/${id}`);
  };
...
- `getRivals(heroId)` returns `string[]`, consumed correctly in Task 8 ✅
