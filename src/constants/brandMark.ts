// src/constants/brandMark.ts — the Mythique mask emblem, extracted once from
// assets/mythique-logo.svg so every share-card renderer draws the SAME logo from
// one source: satori (`<img>` data URI), the web canvases (Image data URI), and
// native (react-native-svg <Path>). Pure data, no RN imports — `api/` may import it.
//
// The mark is a single path whose eye-holes are cut by winding, so recolouring is
// just a fill swap. Pair it gold above the beige wordmark as the header lockup.
...
/** Native viewBox of the extracted path. */
export const MARK_VIEWBOX = '60 345 907 346';
/** width ÷ height — the mark is a wide mask. */
export const MARK_ASPECT = 2.62139;

/** Full standalone SVG markup in the given fill. Carries explicit width/height so
 *  it draws reliably as an `<img>` / canvas image (some engines need intrinsic dims). */
export const mythiqueMarkSvg = (fill: string): string =>
  `<svg xmlns="http://www.w3.org/2000/svg" width="907" height="346" viewBox="${MARK_VIEWBOX}"><path fill="${fill}" d="${MARK_PATH}"/></svg>`;

/** A `data:image/svg+xml` URI (satori `<img>` + canvas `Image`). */
export const mythiqueMarkDataUri = (fill: string): string =>
  `data:image/svg+xml,${encodeURIComponent(mythiqueMarkSvg(fill))}`;
