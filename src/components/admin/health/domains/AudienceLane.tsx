...
// analytics, the visitor explorer (who / where / how / on what), the full
// activity history, acquisition, community engagement, and the client-error
// feed as sub-tabs of one read-only lane. Each sub-tab's query runs only while
// it's active; query keys match the old page-level ones so nothing else changes.
import { useState } from 'react';
import { useUrlTabState } from '../../../../hooks/useUrlTabState';
import { useQuery } from '@tanstack/react-query';
import { SubTabs } from '../SubTabs';
import { TrafficDomain } from './TrafficDomain';
import { VisitorsDomain } from './VisitorsDomain';
import { ActivityHistoryDomain } from './ActivityHistoryDomain';
...
export type AudienceSub =
  'traffic' | 'visitors' | 'activity' | 'acquisition' | 'community' | 'errors';
...
export function AudienceLane({
  ...
  const [activityKind, setActivityKind] = useState<ActivityFilter>('all');
    ...
    <>
      <SubTabs<AudienceSub>
        tabs={[
          ...
          { key: 'visitors', label: 'Visitors', icon: 'earth-outline' },
      ...
      {sub === 'visitors' ? (
        <VisitorsDomain narrow={narrow} days={trafficDays} onDaysChange={setTrafficDays} />
      ) : null}
      {sub === 'activity' ? (
        <ActivityHistoryDomain narrow={narrow} kind={activityKind} onKindChange={setActivityKind} />
      ) : null}
      {sub === 'acquisition' ? (
        <AcquisitionDomain
          data={trafficQ.data ?? null}
