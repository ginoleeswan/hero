    ...
    <View style={styles.spot}>
      <View style={styles.frame}>
        ...
        {spot.hero.portrait ? (
          ...
          <View style={styles.frameOverlay}><ActivityIndicator color="#fff" /></View>
        )}
      </View>
      <View style={styles.spotBody}>
        <Text style={styles.spotKicker}>{generating ? 'PAINTING NOW' : 'PORTRAIT READY'}</Text>
        <Text style={styles.spotName} numberOfLines={2}>{spot.hero.name}</Text>
...
const styles = StyleSheet.create({
  ...
  // Spotlight
  spot: {
    flexDirection: 'row', gap: 16, padding: 14, borderRadius: 14,
    backgroundColor: '#fff', borderWidth: 1, borderColor: 'rgba(41,60,67,0.08)',
  },
  frame: {
    width: 96, height: 124, borderRadius: 12, overflow: 'hidden',
    backgroundColor: '#ece2cf',
  },
  frameImg: { width: '100%', height: '100%' },
  frameOverlay: {
    position: 'absolute', top: 0, left: 0, right: 0, bottom: 0,
    alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(11,18,24,0.35)',
  },
