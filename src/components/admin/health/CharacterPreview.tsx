...
export function CharacterPreview({
  detail, loading, fallbackImage, inCat, busy, onAdd,
}: {
  ...
  const firstSeen = fa ? [fa.name, fa.issueNumber ? `#${fa.issueNumber}` : null].filter(Boolean).join(' ') : null;
  const facts: { label: string; value: string }[] = [];
  if (detail.realName) facts.push({ label: 'Real name', value: detail.realName });
  if (detail.appearances != null) facts.push({ label: 'Appearances', value: detail.appearances.toLocaleString() });
    ...
    <View style={styles.wrap}>
        ...
        <View style={styles.facts}>
          ...
          {detail.aliases.length > 0 ? (
            <View style={styles.factRow}>
              ...
              <Text style={styles.factValue} numberOfLines={1}>{detail.aliases.slice(0, 4).join(', ')}</Text>
            </View>
          ) : null}
        </View>
      </View>

      {detail.deck ? <Text style={styles.deck} numberOfLines={4}>{detail.deck}</Text> : null}
      ...
      >
        {busy ? (
          ...
          <Ionicons name={inCat ? 'checkmark' : 'add'} size={15} color={inCat ? COLORS.green : '#fff'} />
        )}
    ...
    <View style={styles.tagBlock}>
      ...
      <View style={styles.tags}>
        {tags.slice(0, max).map((t) => (
          <View key={t} style={styles.tag}><Text style={styles.tagText} numberOfLines={1}>{t}</Text></View>
        ))}
...
const styles = StyleSheet.create({
  ...
  factLabel: { width: 86, fontFamily: 'Nunito_700Bold', fontSize: 11, color: COLORS.grey, textTransform: 'uppercase', letterSpacing: 0.3 },
  factValue: { flex: 1, minWidth: 0, fontFamily: 'Nunito_700Bold', fontSize: 12.5, color: COLORS.black },

  deck: { fontFamily: 'Nunito_400Regular', fontSize: 12.5, color: COLORS.navy, lineHeight: 18 },

  tagBlock: { gap: 5 },
  tagLabel: { fontFamily: 'Nunito_700Bold', fontSize: 11, color: COLORS.grey, textTransform: 'uppercase', letterSpacing: 0.3 },
  tags: { flexDirection: 'row', flexWrap: 'wrap', gap: 5, alignItems: 'center' },
  tag: { backgroundColor: COLORS.orange + '14', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  addBtn: {
    ...
    backgroundColor: COLORS.orange, borderRadius: 9, paddingVertical: 9, marginTop: 2,
  },
