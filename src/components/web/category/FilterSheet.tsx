...
export function FilterSheet({
  open, slug, filters, counts, setFilter, onReset, onClose, total, hasActive,
}: Props) {
  ...
  return (
    <View style={s.overlay as object}>
      ...
      <Animated.View style={[s.sheet, { transform: [{ translateY }] }] as object}>
        ...
        <ScrollView style={s.body} contentContainerStyle={s.bodyContent as object} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.footer as object}>
          <Pressable
            onPress={onClose}
            style={({ hovered }: { hovered?: boolean }) => [s.apply, hovered && (s.applyHover as object)] as object}
          >
...
const s = StyleSheet.create({
  ...
  sheet: {
    ...
    paddingHorizontal: 20, paddingTop: 10, paddingBottom: 20, maxHeight: '82%', gap: 16,
  } as object,
  grab: {
    alignSelf: 'center', width: 42, height: 5, borderRadius: 3,
    backgroundColor: 'rgba(245,235,220,0.22)', marginBottom: 4,
  } as object,
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  title: { fontFamily: 'Flame-Regular', fontSize: 22, color: COLORS.beige } as object,
  clearText: { fontFamily: 'Nunito_700Bold', fontSize: 13, color: COLORS.orange, letterSpacing: 0.2 } as object,
  body: { flexGrow: 0 },
  ...
  applyHover: { transform: [{ translateY: -1 }], boxShadow: '0 16px 36px -8px rgba(231,115,51,0.75)' } as object,
  applyText: { fontFamily: 'Nunito_900Black', fontSize: 15.5, color: '#fff', letterSpacing: 0.3 } as object,
});
